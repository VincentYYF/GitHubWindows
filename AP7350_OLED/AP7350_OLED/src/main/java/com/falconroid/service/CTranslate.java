...
public class CTranslate {
	...
	private com.falconroid.service.CByteRecord m_recordNumber;
	private Paint paint;
...
//		saveMyBitmap(bitmap,"bb"+System.currentTimeMillis());
	...
	public CByteRecord getQRCode(String data,int x,int y,int width) throws WriterException {
		if(m_recordNumber == null){
			m_recordNumber = new CByteRecord();
		}
		Bitmap bitmap = null;
		bitmap = EncodingHandler.createQRCode(data, width);
		return getBitmap(bitmap, x, y);
	}
	
	
	public CByteRecord getBarcode(String data,int x,int y,int width,int height,boolean isTextView){
		if(m_recordNumber == null){
			m_recordNumber = new CByteRecord();
		}
		Bitmap bitmap = EncodingHandler.creatBarcode(mContext, data, width, height, isTextView);
		return getBitmap(bitmap, x, y);
	}
